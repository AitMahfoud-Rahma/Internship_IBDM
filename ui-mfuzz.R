navbarPage("Mfuzz clustering", 
           ...
           tabPanel(
             ...
             # Adding the radio button for tool selection
             selectInput(
               inputId = "enrichmentTool",
               label = "Choose enrichment tool:",
             ...
             fluidPage(
               ...
               # Example for downloading inertia plot
               downloadInertia <- downloadHandler(
                 filename = function() {
                   "inertia_plot.png"
                 },
                 content = function(file) {
                   # Your code here to generate the inertia plot and save it to the file
                   # e.g., ggsave(file, plot_object)
                 }
               ),
               # Example for downloading Mfuzz plots
               downloadMfuzz <- downloadHandler(
                 filename = function() {
                   "mfuzz_plots.zip" # You can choose any desired filename
                 },
                 content = function(file) {
                   # Your code here to generate the Mfuzz plots and save them to the zip file
                   # e.g., saveMfuzzPlots(file)
                 }
               ),
               # Example for downloading cluster enrichment results
               downloadEnrichment <- downloadHandler(
                 filename = function() {
                   "cluster_enrichment_results.csv" # You can choose any desired filename
                 },
                 content = function(file) {
                   # Your code here to generate the cluster enrichment results and save them to the file
                   # e.g., write.csv(enrichment_results, file)
                 }
               )
             )))